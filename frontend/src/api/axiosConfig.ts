      ...
      if (error.response) {
        if (error.response.status === 401 && !originalRequest._retry) {
          ...
          try {
            ...
            errorMessage = 'User verification failed. Token is invalid or expired. Please log in again.';
            localStorage.removeItem('access_token');
            localStorage.removeItem('refresh_token');
            window.location.href = '/login';
          }
        } else if (error.response.status === 404) {
          errorMessage = 'The requested resource was not found. Please contact the administrator.';
        } else {
